...
impl Bus {
    ...
    fn debug(&mut self) {
        unsafe {
            println!("ATA status register: 0b{:08b} <BSY|DRDY|#|#|DRQ|#|#|ERR>", self.alternate_status_register.read());
            println!("ATA error register:  0b{:08b} <#|#|#|#|#|ABRT|#|#>", self.error_register.read());
        }
    }
}
